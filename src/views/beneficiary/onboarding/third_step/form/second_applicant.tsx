...
type secondApplicantFormData = z.infer<typeof secondApplicantSchema>;

export default function SecondApplicantForm() {
  const {
    handleSubmit,
    control,
    setValue,
    formState: {},
  } = useForm<secondApplicantFormData>({
  ...
  const onSubmit = (data: secondApplicantFormData) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <p className="text-text-dark text-xs font-plus_jakarta_sans">
        Hi Boma, upload all required documents to be reviewed for approval and
        grant payment.
      </p>
