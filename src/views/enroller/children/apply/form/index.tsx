...
import { addLoanSchema } from "./schema";
import ScannerIcon from "@/components/icons/ScannerIcon";

type FormValues = z.infer<typeof addLoanSchema>;
const options = [
  { id: "nin", name: "NIN" },
...
export default function EnrollerApplyForLoanForm() {
  const {
    control,
    handleSubmit,
    setValue,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-5">
        {" "}
        <FormLayout
          heading="Select Parent"
          subHeading="Select Parent Bio Data related to Child’s Loan Application"
        >
          <ControlledSelect
            name="education"
            control={control}
      ...
      >
        <div className="grid grid-cols-3 gap-x-4 gap-y-6 ">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-text-dark font-plus_jakarta_sans text-sm text-center">
              No Data Captured yet
            </p>
            <Button className="py-[0.6rem]">Capture Biometrics</Button>
          </div>
        ...
        <section className="grid grid-cols-3 gap-2">
          ...
          <div className="lg:w-72 flex flex-col space-y-2">
            <div className="border p-4 lg:w-72 h-[14rem] rounded-md flex flex-col gap-3 text-text-dark transition-colors border-neutral-300">
              ...
              />
              <p className="font-plus_jakarta_sans text-xs">
                <span className="font-semibold">Note:</span>2.5% upon total loan
                would be added as interest upon repayment within a 4 month
                period
              </p>
            </div>
